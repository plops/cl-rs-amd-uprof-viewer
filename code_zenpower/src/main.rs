...
fn read_int (data: &[u8]) -> io::Result<u64>{
        ...
        for  (i, byte) in data.iter().enumerate() {
                match byte {
                        ...
                        0 => {
                        {
                        println!("{} {}:{} parse zero  i={:?}  byte={:?}", Utc::now(), file!(), line!(), i, byte);
}
                                res=parse(&(data[0..i]));
},
                        _ => {
                        {
                        println!("{} {}:{} parse error  i={:?}  byte={:?}", Utc::now(), file!(), line!(), i, byte);
}
...
fn main (){
...
}
    {
        ...
        b.spawn(move ||{
                                                let f_SVI2_C_Core  = File::open("/sys/devices/pci0000:00/0000:00:18.3/hwmon/hwmon0/curr1_input").unwrap();
            let f_SVI2_C_SoC  = File::open("/sys/devices/pci0000:00/0000:00:18.3/hwmon/hwmon0/curr2_input").unwrap();
            let f_SVI2_Core  = File::open("/sys/devices/pci0000:00/0000:00:18.3/hwmon/hwmon0/in1_input").unwrap();
            let f_SVI2_SoC  = File::open("/sys/devices/pci0000:00/0000:00:18.3/hwmon/hwmon0/in2_input").unwrap();
            let f_SVI2_P_Core  = File::open("/sys/devices/pci0000:00/0000:00:18.3/hwmon/hwmon0/power1_input").unwrap();
            let f_SVI2_P_SoC  = File::open("/sys/devices/pci0000:00/0000:00:18.3/hwmon/hwmon0/power2_input").unwrap();
            let f_Tdie  = File::open("/sys/devices/pci0000:00/0000:00:18.3/hwmon/hwmon0/temp1_input").unwrap();
            let f_Tctl  = File::open("/sys/devices/pci0000:00/0000:00:18.3/hwmon/hwmon0/temp2_input").unwrap();
            let f_Tccd1  = File::open("/sys/devices/pci0000:00/0000:00:18.3/hwmon/hwmon0/temp3_input").unwrap();
            loop {
                ...
                let mut buf_Tccd1  = [0; 32];
                                let _bytes_SVI2_C_Core  = f_SVI2_C_Core.read_at(0, &mut buf_SVI2_C_Core).expect("read_at SVI2_C_Core fail");
                let _bytes_SVI2_C_SoC  = f_SVI2_C_SoC.read_at(0, &mut buf_SVI2_C_SoC).expect("read_at SVI2_C_SoC fail");
                let _bytes_SVI2_Core  = f_SVI2_Core.read_at(0, &mut buf_SVI2_Core).expect("read_at SVI2_Core fail");
                let _bytes_SVI2_SoC  = f_SVI2_SoC.read_at(0, &mut buf_SVI2_SoC).expect("read_at SVI2_SoC fail");
                let _bytes_SVI2_P_Core  = f_SVI2_P_Core.read_at(0, &mut buf_SVI2_P_Core).expect("read_at SVI2_P_Core fail");
                let _bytes_SVI2_P_SoC  = f_SVI2_P_SoC.read_at(0, &mut buf_SVI2_P_SoC).expect("read_at SVI2_P_SoC fail");
                ...
                let _bytes_Tccd1  = f_Tccd1.read_at(0, &mut buf_Tccd1).expect("read_at Tccd1 fail");
                ...
                s.send((Utc::now(), v_SVI2_C_Core, v_SVI2_C_SoC, v_SVI2_Core, v_SVI2_SoC, v_SVI2_P_Core, v_SVI2_P_SoC, v_Tdie, v_Tctl, v_Tccd1)).unwrap();
};
});
};
        {
        ...
        system.main_loop(move | _,  ui|{
...
}
                {
                    ...
                    ui.plot_lines(&label, &(data_time_between_samples_ms)).build();
};
});
});
}
